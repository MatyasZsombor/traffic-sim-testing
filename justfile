...
build-tauri-signed-linux:
  cargo tauri build --config '{"bundle":{"targets":["deb","rpm","appimage"]},"plugins":{"updater":{"active":true}}}'

[working-directory: 'app/src-tauri']
build-tauri-signed-windows:
  cargo tauri build --config '{\"bundle\":{\"targets\":[\"msi\",\"nsis\"]},\"plugins\":{\"updater\":{\"active\":true}}}'
